...
/*
  💣 이 컴포넌트에서 input창에 입력할때마다 try가 리렌더링 되는 이슈 발생!

  그 이유는?
  💡 부모 컴포넌트가 리랜더링되면, 반드시 자식 컴포넌트도 리랜더링 되기 때문!!

  종합해서 - 컴포넌트가 리랜더링 되는 경우
  1.state가 바뀐다. 2.props가 바뀐다. 3.부모컴포넌트가 바뀐다.

  이런 어울한 자식 컴포넌트의 리렌더링을 막아줄수 있는게
  PureComponent이다.

  PureComponent는 아래의 두가지 기능이 있다.
  1.state가 달라졌을 때만 리렌더링되게 하는 기능
  2.props가 달라졌을 때만 리렌더링되게 하는 기능

  근데!!💥
  PureComponent는 class Component일 때만 사용가능한데??
  함수 컴포넌트에서도 이를 사용할 수 있도록 한 것이 있다. 
  그게바로 memo!! >> Try.jsx에서 memo 확인

  아니 PureComponent 일케 좋은데 왜 그냥 Component써?
  할수도 있지만!

  실제론 Component에서 '이 state는 바뀌어도 절대 리랜더링 안되게 할거야'
  등등의 커스텀이 다양해서
  실무에선 그냥 Component도 많이 쓴다.

  📌 요약
  컴포넌트 랜더링 최적화 할때
  랜더링 커스텀이 필요하다: 그냥 컴포넌트 + shouldComponentUpdate
  커스텀 필요없고 부모로인해 자식까지 리랜더링 되는거 막고 싶다: 함수형은 memo, 클래스는 PureComponent
*/ 
const NumberBaseball = () => {
  const [result, setResult] = useState('');
  const [value, setValue] = useState('');
